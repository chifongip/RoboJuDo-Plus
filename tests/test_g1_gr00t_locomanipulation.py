...
class TestG1Gr00tLocomanipulation(unittest.TestCase):
    def test_configs_match_23_dof_profiles(self):
        from robojudo.config.g1.g1_vla_cfg import g1_23_gr00t_locomanipulation_stiff_real

        cfg = g1_23_gr00t_locomanipulation_stiff_real()
        ...
        self.assertEqual(cfg.policy.pd_gain_preset, "stiff")
        self.assertEqual(cfg.policy.policy_name, "policy_23dof_stiff")
        ...
        self.assertEqual(len(cfg.ctrl[-1].upper_body_default_pose), 10)
        self.assertTrue(cfg.ctrl[-1].observation_enabled)
        self.assertEqual(cfg.ctrl[-1].observation_profile, "g1_23dof")
        self.assertEqual(cfg.ctrl[-1].camera.type, "opencv")
        self.assertEqual(cfg.ctrl[-1].camera.options["device"], 0)
        self.assertIsNotNone(cfg.ctrl[-1].casia_hand)
        self.assertEqual([controller.ctrl_type for controller in cfg.ctrl], ["UnitreeCtrl", "Gr00tZmqCtrl"])

    def test_casia_vla_configs_are_real_only(self):
        from robojudo.config import cfg_registry

        self.assertNotIn("g1_23_gr00t_locomanipulation_stiff", cfg_registry.types)
        self.assertIn("g1_23_gr00t_locomanipulation_stiff_real", cfg_registry.types)
        self.assertIn("g1_23_casia_locomanipulation_stiff_real", cfg_registry.types)
    ...
    def test_stiff_joint_default_uses_gr00t_upper_body_pose(self):
    ...
    def test_real_stiff_lower_body_model_accepts_gr00t_commands(self):
        ...
        cfg = G1Gr00tLocomanipulation23PolicyCfg(
