...
class X2LocomanipulationPolicy(Policy):
    cfg_policy: X2LocomanipulationPolicyCfg

    def __init__(self, cfg_policy: X2LocomanipulationPolicyCfg, device: str):
        ...
        logger.info("Loaded X2 locomanipulation ONNX model from %s", cfg_policy.policy_file)
            ...
            if key == "KeyboardCtrl":
                ...
                for held_key in self._held_keys:
                    if held_key in vel_keys:
                        ...
                        magnitude = high if sign > 0 else abs(low)
                        target_vel[axis] += sign * magnitude
    ...
    def get_observation(self, env_data, ctrl_data):
        ...
        for key, expected_dim in self.cfg_policy.history_obs_dims.items():
            ...
            if term.shape != (expected_dim,):
                raise ValueError(
                    f"X2 locomanipulation observation term {key} has shape {term.shape}, "
        ...
        if obs.shape != (self.cfg_policy.num_obs,):
            raise ValueError(
                f"X2LocomanipulationPolicy observation shape {obs.shape} != ({self.cfg_policy.num_obs},)"
            )
        if not np.isfinite(obs).all():
            raise FloatingPointError("X2 locomanipulation observation contains non-finite values")
        return obs, {}
    ...
    def get_action(self, obs: np.ndarray) -> np.ndarray:
        ...
        if model_obs.shape != (1, self.cfg_policy.num_obs):
            raise ValueError(
                f"X2 locomanipulation ONNX observation shape {model_obs.shape} "
        ...
        if raw_action.shape != (self.num_actions,):
            raise ValueError(
                f"X2 locomanipulation action shape {raw_action.shape} != ({self.num_actions},)"
            )
        if not np.isfinite(raw_action).all():
            raise FloatingPointError("X2 locomanipulation policy produced a non-finite action")
    ...
    def _validate_model_contract(self):
        ...
        if input_shapes != {"obs": [1, self.cfg_policy.num_obs]}:
            raise ValueError(f"Unexpected X2 locomanipulation ONNX inputs: {input_shapes}")
        if output_shapes.get("actions") != [1, self.num_actions]:
            raise ValueError(f"Unexpected X2 locomanipulation ONNX outputs: {output_shapes}")

        metadata = self.session.get_modelmeta().custom_metadata_map
        joint_names = metadata.get("joint_names", "").split(",")
        if joint_names != self.cfg_obs_dof.joint_names:
            raise ValueError("X2 locomanipulation ONNX joint_names do not match the policy configuration")
        observation_names = metadata.get("observation_names", "").split(",")
        if observation_names != list(self.cfg_policy.history_obs_dims):
            raise ValueError("X2 locomanipulation ONNX observation_names do not match the policy configuration")
        command_names = metadata.get("command_names", "").split(",")
        if command_names != ["twist", "base_height", "waist_yaw"]:
            raise ValueError(f"Unexpected X2 locomanipulation command metadata: {command_names}")
    ...
    @staticmethod
    def _validate_rounded_metadata(metadata: dict[str, str], key: str, expected) -> None:
        actual = np.asarray([float(value) for value in metadata.get(key, "").split(",")], dtype=np.float64)
        expected_array = np.asarray(expected, dtype=np.float64)
        if actual.shape != expected_array.shape or not np.allclose(actual, expected_array, rtol=0.0, atol=5e-4):
            raise ValueError(f"X2 locomanipulation ONNX {key} metadata does not match the policy configuration")
