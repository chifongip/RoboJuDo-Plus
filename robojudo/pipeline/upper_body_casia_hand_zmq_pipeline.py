...
class UpperBodyCasiaHandZmqPipelineMixin:
    ...
    def _set_upper_body_enabled(self, enabled: bool):
        ...
        if was_enabled == is_enabled:
            return
        controller = self.ctrl_manager.controllers.get("UpperBodyCasiaHandZmqCtrl")
    ...
    def _apply_pd_target_override(self, pd_target: np.ndarray, ctrl_data) -> np.ndarray:
        stream_data = ctrl_data.get("UpperBodyCasiaHandZmqCtrl")
    ...
    def _post_mode_step(self, env_data, ctrl_data, extras, pd_target, rl_active: bool):
        self._upper_body_casia_hand_ctrl_data = ctrl_data
        try:
            return super()._post_mode_step(env_data, ctrl_data, extras, pd_target, rl_active)
        finally:
            self._upper_body_casia_hand_ctrl_data = None
    ...
    def _record_upper_body_sample(self, env_data, extras, pd_target, *, rl_active: bool):
        ...
        ctrl_data = getattr(self, "_upper_body_casia_hand_ctrl_data", None) or {}
        hand_data = ctrl_data.get("UpperBodyCasiaHandZmqCtrl", {}).get("casia_hand")
        ...
        if not hand_names or hand_positions.shape != expected_shape or hand_commands.shape != expected_shape:
            logger.warning("Skipped recording frame with an invalid CASIA Hand snapshot")
            return
